...
#            stats['timeout']=1
#        else:
#            raise "Can't determine the problem status.!!!!"
#    return stats
    ...
    for l in lns:
        ...
        if l != '':
            if l.endswith('.intohylo'):
                benchmark = os.path.basename(os.path.dirname(l))
                if benchmark not in data:
                    data[benchmark]={}
                key=os.path.basename(l)
                data[benchmark][key]=[l]
                continue
            data[benchmark][key].append(l)
    return data
...
#    data=[]
    for k,d in plns.iteritems():
        print k, len(d.keys())
        vals= [item for sublist in d.values() for item in sublist]
        f_out = ('%s-%s.out' % (os.path.basename(filename),k)) 
        path_out = os.path.join(args.output, f_out)
        list_to_file(f_out, vals)
